...
// Description : ABCDEF coordinate randomized parallel
//============================================================================
		...
		int m, int n, int d, int n_threads, char *funcname)
{
	...
	while (nu>tolnu  &&  k<maxiter)
	{
		for(j = 0;j<inneriter;j++)  // inner loop
		{
            ...
            #pragma omp parallel proc_bind(spread) num_threads(n_threads) //shared(x, AT, A, Y, nu, CT, s, H, lambda, theta)
			{
				...
				{
					int idx = rand()%d;
					double x_temp = x(idx,0)-( ((AT.row(idx))*((*A)*x-(*Y)))(0,0) + nu*((CT.row(idx))*s)(0,0) )/H;
					double xtt = 0;
